...
public class AlipayServiceImpl implements AlipayService {
    ...
    public void aliPay(HttpServletResponse response, HttpServletRequest request) throws IOException {
        ...
        String order_number = new String(order.getOrderNo().toString());
        //付款金额，从前台获取，必填
        String total_amount = new String(order.getTotalSum().toString());
        //订单名称，必填
        String subject = new String("网上书城");
        //商品描述，可空
        String body = "";
        for (int i=0;i<order.getOrderDetailList().size();i++){
            body += order.getOrderDetailList().get(i).getProductName()+",";
        }
...
//        log.info(notifyBuild.toString());
    ...
    public void aliPayByOrderNo(HttpServletResponse response, HttpServletRequest request,String orderNo) throws IOException{
        //获得初始化的AlipayClient
        AlipayClient alipayClient = new DefaultAlipayClient(AlipayConfig.gatewayUrl, AlipayConfig.app_id, AlipayConfig.merchant_private_key, "json", AlipayConfig.charset, AlipayConfig.alipay_public_key, AlipayConfig.sign_type);
        //设置请求参数
        AlipayTradePagePayRequest aliPayRequest = new AlipayTradePagePayRequest();
        ...
        Orders order = orderService.getOrderByNo(orderNo);
        System.out.println(order.toString());
        String order_number = new String(order.getOrderNo());
        //付款金额，从前台获取，必填
        String total_amount = new String(order.getTotalSum().toString());
        //订单名称，必填
        String subject = new String("网上书城");
